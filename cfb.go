...
func IsOleBytes(b []byte) (bool, error) {
	f, err := NewFileFromBytes(b)
	if err != nil {
		return false, err
	}
	if f.Size() < len(MAGIC) {
...
func NewOleFileFromBytes(b []byte) (this *OleFile, err error) {
	this = &OleFile{
		used_streams_fat:     make([]uint32, 0, 10),
		used_streams_minifat: make([]uint32, 0, 10),
	}

	this.fp, err = NewFileFromBytes(b)
	if err != nil {
		return nil, err
	}
...
func (o *OleFile) Open(filename string) (err error) {
	if o.fp, err = NewFile(filename); err != nil {
		return
	}
...
func (o *OleFile) load() error {
	o.header = new(headerFields)
	err := binary.Read(o.fp, binary.LittleEndian, o.header)
	if err != nil {
		return err
	}
	if !bytes.Equal(MAGIC, o.header.Signature[:]) {
		return ErrorStructure
	}
	if o.header.MajorVersion != 3 && o.header.MajorVersion != 4 {
		return fmt.Errorf("Incorrect Version in OLE header: %v", o.header.MajorVersion)
	}
	if o.header.ByteOrder != 0xFFFE {
		return fmt.Errorf("Incorrect ByteOrder in OLE header: %v", o.header.ByteOrder)
	}
	o.sector_size = uint32(1 << o.header.SectorShift)
	if o.sector_size != 512 && o.sector_size != 4096 {
		return fmt.Errorf("Incorrect sector_size in OLE header (expected: 512 or 4096): %v", o.sector_size)
	}
	if (o.header.MajorVersion == 3 && o.sector_size != 512) ||
		(o.header.MajorVersion == 4 && o.sector_size != 4096) {
		return errors.New("sector_size does not match DllVersion in OLE header")
	}
	o.mini_sector_size = uint32(1 << o.header.MiniSectorShift)
	if o.mini_sector_size != 64 {
		return fmt.Errorf("Incorrect mini_sector_size in OLE header: %v", o.mini_sector_size)
	}
	if o.sector_size == 512 && o.header.NumDirSectors != 0 {
		return errors.New("Incorrect number of directory sectors in OLE header")
	}
	if o.header.TransactionSignatureNumber != 0 {
		return errors.New("Incorrect OLE header (transaction_signature_number>0)")
	}
	if o.header.MiniStreamCutoffSize != 0x1000 {
		return errors.New("Incorrect mini_stream_cutoff_size in OLE header")
	}
	// calculate the number of sectors in the file
	// (-1 because header doesn't count)
	o.nb_sect = ((o.fp.Size() + int(o.sector_size) - 1) / int(o.sector_size)) - 1

	if err = o.check_duplicate_stream(o.header.FirstDirSector, false); err != nil {
		return err
	}
	if o.header.NumMiniFatSectors > 0 {
		if err = o.check_duplicate_stream(o.header.FirstMiniFatSector, false); err != nil {
			return err
		}
	}
	if o.header.NumDifatSectors > 0 {
		if err = o.check_duplicate_stream(o.header.FirstDifatSector, false); err != nil {
			return err
		}
	}

	// Load file allocation tables
	if err = o.loadfat(); err != nil {
		return err
	}
	// Load directory.  This sets both the direntries list (ordered by sid)
	// and the root (ordered by hierarchy) members.
	if err = o.loaddirectory(o.header.FirstDirSector); err != nil {
		return err
	}
	//o.minifatsect = o.header.FirstMiniFatSector

	return nil
}

func (o *OleFile) Close() {
	if o != nil && o.fp != nil {
		o.fp.Close()
	}
}

func (o *OleFile) check_duplicate_stream(first_sect uint32, minifat bool) error {
	var used_streams []uint32
	if minifat {
		used_streams = o.used_streams_minifat
	} else {
		if first_sect == DIFSECT || first_sect == FATSECT || first_sect == ENDOFCHAIN || first_sect == FREESECT {
			return nil
		}
		used_streams = o.used_streams_fat
	}
...
func (o *OleFile) loadfat() (err error) {
	...
	if o.header.NumDifatSectors != 0 {
		...
		if int(o.header.FirstDifatSector) >= o.nb_sect {
		...
		nb_difat_sectors := (o.sector_size / 4) - 1
		nb_difat := (o.header.NumFatSectors - 109 + nb_difat_sectors - 1) / nb_difat_sectors
		logger.Debugf("nb_difat = %d\n", nb_difat)
		if o.header.NumDifatSectors != nb_difat {
			err = errors.New("Incorrect DIFAT")
			return
		}
		isect_difat := o.header.FirstDifatSector
		for i := uint32(0); i < nb_difat; i++ {
			var sector_difat []byte
			var difat []uint32

			logger.Debugf("DIFAT block %d, sector %X\n", i, isect_difat)
			if sector_difat, err = o.getsect(isect_difat); err != nil {
				return
			}
			if difat, err = sect2array(sector_difat); err != nil {
				return
			}
			if _, err = o.loadfat_sect(difat[:nb_difat_sectors]); err != nil {
				return
			}
	...
	if len(o.fat) > o.nb_sect {
		logger.Debugf("len(fat)=%d, shrunk to nb_sect=%d\n", len(o.fat), o.nb_sect)
		o.fat = o.fat[:o.nb_sect]
	}
	logger.Debugf("FAT references %d sectors / Maximum %d sectors in file", len(o.fat), o.nb_sect)

	return
}

// Adds the indexes of the given sector to the FAT
func (o *OleFile) loadfat_sect(sect []uint32) (isect uint32, err error) {
	for _, isect = range sect {
		...
		// read the FAT sector
		if s, err = o.getsect(isect); err != nil {
			return
		}
		...
		o.fat = append(o.fat, nextfat...)
	}
	return
}

// Read given sector from file on disk.
func (o *OleFile) getsect(sect uint32) (sector []byte, err error) {
	if _, err = o.fp.Seek(int(o.sector_size*(sect+1)), 0); err != nil {
		return
	}
	sector = make([]byte, o.sector_size)
	var n int
	if n, err = o.fp.Read(sector); err != nil {
		return
	}
	if n != int(o.sector_size) {
...
// Load the directory.
func (o *OleFile) loaddirectory(sect uint32) (err error) {
	logger.Debug("Loading the Directory:")

	if o.directory_fp, err = o.open(sect, UNKNOWN_SIZE, true); err != nil {
		return
	}
	max_entries := o.directory_fp.size / 128

	logger.Debugf("loaddirectory: size=%d, max_entries=%d",
		o.directory_fp.size, max_entries)

	o.direntries = make([]*OleDirectoryEntry, max_entries)

	if _, err = o.load_direntry(0); err != nil {
		return
	}
	o.root = o.direntries[0]
	o.root.build_storage_tree()
	return
}

// Load a directory entry from the directory.
func (o *OleFile) load_direntry(sid int) (de *OleDirectoryEntry, err error) {
	if sid < 0 || sid >= len(o.direntries) {
		err = errors.New("OLE directory index out of range")
		return
	}
	if o.direntries[sid] != nil {
		err = errors.New("double reference for OLE stream/storage")
		return
	}
	o.directory_fp.Seek(sid*128, 0)
	entry := make([]byte, 128)
	if _, err = o.directory_fp.Read(entry); err != nil {
		return
	}
	o.direntries[sid], err = NewOleDirectoryEntry(entry, sid, o)
	de = o.direntries[sid]
	return
}

// Open a stream, either in FAT or MiniFAT according to its size.
func (o *OleFile) open(start, size uint32, force_FAT bool) (stream *OleStream, err error) {
	logger.Debugf("OleFile.open(): sect=%Xh, size=%d, force_FAT=%v",
		start, size, force_FAT)

	if size < o.header.MiniStreamCutoffSize && !force_FAT {
		// ministream object
		if o.ministream == nil {
			if err = o.loadminifat(); err != nil {
				return
			}
			size_ministream := o.root.size
			logger.Debugf("Opening MiniStream: sect=%Xh, size=%d",
				o.root.header.IsectStart, size_ministream)
			o.ministream, err = o.open(o.root.header.IsectStart, uint32(size_ministream), true)
			if err != nil {
				o.ministream = nil
				return
			}
		}
		stream, err = NewOleStream(o.ministream, start, size,
			0, o.mini_sector_size,
			o.minifat, uint32(o.ministream.size),
			o)
	} else {
		// standard stream
		stream, err = NewOleStream(o.fp, start, size,
			o.sector_size,
			o.sector_size, o.fat,
			uint32(o.fp.Size()),
			o)
...
func (o *OleFile) loadminifat() (err error) {
	stream_size := o.header.NumMiniFatSectors * o.sector_size
	nb_minisectors := (o.root.size + uint64(o.mini_sector_size) - 1) / uint64(o.mini_sector_size)
	...
	logger.Debugf("loadminifat(): minifatsect=%d, nb FAT sectors=%d, used_size=%d, stream_size=%d, nb MiniSectors=%d",
		o.header.FirstMiniFatSector, o.header.NumMiniFatSectors, used_size, stream_size, nb_minisectors)
	...
	var s *OleStream
	if s, err = o.open(o.header.FirstMiniFatSector, stream_size, true); err != nil {
		return
	}
	var b []byte
	...
	if o.minifat, err = sect2array(b); err != nil {
		return
	}
	logger.Debugf("MiniFAT shrunk from %d to %d sectors", len(o.minifat), nb_minisectors)
	o.minifat = o.minifat[:nb_minisectors]
	logger.Debugf("loadminifat(): len=%d", len(o.minifat))
...
//container.
func (o *OleFile) Exists(filename string) bool {
	if _, err := o.find(strings.ToLower(filename)); err != nil {
		return false
	}
	return true
}
...
func (o *OleFile) find(filename string) (sid int, err error) {
...
func (o *OleFile) GetType(filename string) uint8 {
	sid, err := o.find(filename)
	if err != nil {
		return STGTY_EMPTY
	}
	entry, err := o.get_direntry(sid)
	if err != nil {
		return STGTY_EMPTY
	}
...
func (o *OleFile) GetSize(filename string) int {
	sid, err := o.find(filename)
	if err != nil {
		return -1
	}
...
//implementations.
func (o *OleFile) GetRootentryName() string {
	return o.root.name
}

func (o *OleFile) get_direntry(sid int) (de *OleDirectoryEntry, err error) {
	if sid < 0 || sid >= len(o.direntries) {
...
func (o *OleFile) OpenStream(filename string) (*OleStream, error) {
	sid, err := o.find(filename)
	if err != nil {
		return nil, err
	}
	...
	return o.open(entry.header.IsectStart, uint32(entry.size), false)
}

func (o *OleFile) GetHeader() *headerFields {
	return o.header
}

//Write a stream to disk. For now, it is only possible to replace an
//existing stream by data of the same size.
func (o *OleFile) WriteStream(stream_name string, data []byte) (err error) {
	if data == nil {
		err = errors.New("write_stream: data must be a bytes string")
		return
	}
	var sid int
	if sid, err = o.find(stream_name); err != nil {
		return
	}
	var entry *OleDirectoryEntry
	if entry, err = o.get_direntry(sid); err != nil {
		return
	}
	if entry.header.EntryType != STGTY_STREAM {
	...
	if size < int(o.header.MiniStreamCutoffSize) && entry.header.EntryType != STGTY_ROOT {
		return o.write_mini_stream(entry, data)
	}

	sect := entry.header.IsectStart
	// number of sectors to write
	nb_sectors := (size + (int(o.sector_size) - 1)) // self.sectorsize
	logger.Debugf("b_sectors = %d", nb_sectors)
	for i := 0; i < nb_sectors; i++ {
		var data_sector []byte
		if i < (nb_sectors - 1) {
			data_sector = data[i*int(o.sector_size) : (i+1)*int(o.sector_size)]
			if len(data_sector) == int(o.sector_size) {
				err = errors.New("Error len data_sector")
				return
			}
		} else {
			data_sector = data[i*int(o.sector_size):]
			logger.Debugf("write_stream: size=%d sectorsize=%d data_sector=%Xh size%%sectorsize=%d",
				size, o.sector_size, len(data_sector), size%int(o.sector_size))
		}
		if err = o.write_sect(sect, data_sector); err != nil {
			return
		}

		if sect, err = o.get_fat(sect); err != nil {
			return
		}
	}
	if sect != ENDOFCHAIN {
...
func (o *OleFile) get_fat(id uint32) (uint32, error) {
	if int(id) >= len(o.fat) {
...
func (o *OleFile) get_minifat(id uint32) (uint32, error) {
	if int(id) >= len(o.minifat) {
		return 0, errors.New("incorrect OLE MiniFAT, sector index out of range")
	}
	return o.minifat[id], nil
}

func (o *OleFile) write_mini_stream(entry *OleDirectoryEntry, data_to_write []byte) error {
	if entry.sect_chain == nil {
		if err := entry.build_sect_chain(o); err != nil {
			return err
		}
	}
	nb_sectors := len(entry.sect_chain)

	if o.root.sect_chain == nil {
		if err := o.root.build_sect_chain(o); err != nil {
			return err
		}
	}
	block_size := o.sector_size / o.mini_sector_size
	for idx, sect := range entry.sect_chain {
		sect_base := sect / block_size
		sect_offset := sect % block_size
		fp_pos := (o.root.sect_chain[sect_base]+1)*o.sector_size + sect_offset*o.mini_sector_size
		var data_per_sector []byte
		if idx < (nb_sectors - 1) {
			data_per_sector = data_to_write[idx*int(o.mini_sector_size) : (idx+1)*int(o.mini_sector_size)]
		} else {
			data_per_sector = data_to_write[idx*int(o.mini_sector_size):]
		}
		if err := o.write_mini_sect(fp_pos, data_per_sector); err != nil {
			return err
		}
	}
	return nil
...
func (o *OleFile) write_sect(sect uint32, data []byte) error {
	if data == nil {
	...
	if len(data) > int(o.sector_size) {
		return errors.New("Data is larger than sector size")
	}
	_, err := o.fp.Write(data)
	return err
}

//Write given sector to file on disk.
func (o *OleFile) write_mini_sect(fp_pos uint32, data []byte) error {
	if data == nil {
		return errors.New("write_sect: data must be a bytes string")
	}

	if _, err := o.fp.Seek(int(fp_pos), 0); err != nil {
		return err
	}

	len_data := len(data)
	if int(o.mini_sector_size) < len_data {
		return errors.New("Data is larger than sector size")
	}
	_, err := o.fp.Write(data)
	return err
